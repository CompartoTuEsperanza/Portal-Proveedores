...
function statusBadge(status: string) {
  const map: Record<string, { bg: string; text: string; label: string }> = {
    ...
    "en_evaluación": { bg: "bg-white", text: "text-rose-700", label: "En Evaluación" },
    registrado: { bg: "bg-rose-50/50", text: "text-rose-950", label: "Registrado" },
...
export default function MyLots() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-rose-950">Mis Lotes</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white/50 border-b border-rose-100/60">
                ...
                <th className="text-left py-3 px-4 text-rose-900/70 font-semibold">Tipo</th>
                <th className="text-left py-3 px-4 text-rose-900/70 font-semibold">Región</th>
                <th className="text-right py-3 px-4 text-rose-900/70 font-semibold">Peso (KG)</th>
                <th className="text-left py-3 px-4 text-rose-900/70 font-semibold">Proceso</th>
                <th className="text-left py-3 px-4 text-rose-900/70 font-semibold">Fecha Cosecha</th>
                <th className="text-left py-3 px-4 text-rose-900/70 font-semibold">Estado</th>
                <th className="text-center py-3 px-4 text-rose-900/70 font-semibold">Fotos</th>
              </tr>
            </thead>
            <tbody>
              {myLots.map((lot) => (
                <tr key={lot.id} className="border-b border-rose-50/60 hover:bg-white/40 transition-colors">
                  <td className="py-3 px-4 font-mono text-xs text-rose-800/60">{lot.id}</td>
                  <td className="py-3 px-4 font-medium text-rose-950">{lot.product_type}</td>
                  <td className="py-3 px-4 text-rose-900/70">{lot.region}</td>
                  <td className="py-3 px-4 text-right font-medium text-rose-950">
                    {lot.gross_weight.toLocaleString()}
                  </td>
                  <td className="py-3 px-4 text-rose-900/70">{lot.process}</td>
                  <td className="py-3 px-4 text-rose-900/70">{lot.harvest_date}</td>
                  <td className="py-3 px-4">{statusBadge(lot.status)}</td>
                  <td className="py-3 px-4 text-center text-rose-800/60">
                    {lot.photos.length > 0 ? (
