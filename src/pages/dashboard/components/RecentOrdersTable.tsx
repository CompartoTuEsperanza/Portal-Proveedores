...
export default function RecentOrdersTable({ orders }: RecentOrdersTableProps) {
  ...
  if (orders.length === 0) {
    return (
      <div className="text-center py-8 text-rose-200/40 text-sm">
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-rose-50/60">
            <th className="text-left py-2.5 px-3 text-rose-200/70 font-medium">Orden</th>
            <th className="text-left py-2.5 px-3 text-rose-200/70 font-medium">Fecha</th>
            <th className="text-left py-2.5 px-3 text-rose-200/70 font-medium">Estado</th>
            <th className="text-right py-2.5 px-3 text-rose-200/70 font-medium">Total</th>
          </tr>
        </thead>
        <tbody>
          {orders.map((order, index) => (
            ...
            >
              <td className="py-2.5 px-3 font-medium text-rose-50">{order.id}</td>
              <td className="py-2.5 px-3 text-rose-100/80">{order.date}</td>
              <td className="py-2.5 px-3">{statusBadge(order.status)}</td>
              <td className="py-2.5 px-3 text-right font-medium text-rose-50">
