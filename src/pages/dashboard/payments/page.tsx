...
export default function Payments() {
  ...
  function statusBadge(status: string) {
    const map: Record<string, { bg: string; text: string; label: string }> = {
      enviado: { bg: "bg-white", text: "text-rose-700", label: "Enviado" },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-rose-950">Estado de Pagos</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white/50 border-b border-rose-100/60">
                <th className="text-left py-3 px-4 text-rose-900/70 font-semibold">Lote</th>
                <th className="text-left py-3 px-4 text-rose-900/70 font-semibold">Porcentaje</th>
                <th className="text-right py-3 px-4 text-rose-900/70 font-semibold">Monto</th>
                <th className="text-left py-3 px-4 text-rose-900/70 font-semibold">Banco</th>
                <th className="text-left py-3 px-4 text-rose-900/70 font-semibold">Fecha</th>
                <th className="text-left py-3 px-4 text-rose-900/70 font-semibold">Estado</th>
              </tr>
            </thead>
            <tbody>
              {myPayments.map((payment) => (
                <tr key={payment.id} className="border-b border-rose-50/60 hover:bg-white/40 transition-colors">
                  <td className="py-3 px-4 font-medium text-rose-950">{payment.lot_name}</td>
                  <td className="py-3 px-4 text-rose-900/70">{payment.percentage}%</td>
                  <td className="py-3 px-4 text-right font-semibold text-rose-950">
                    ${payment.amount.toLocaleString()}
                  </td>
                  <td className="py-3 px-4 text-rose-900/70">{payment.bank}</td>
                  <td className="py-3 px-4 text-rose-900/70">{payment.date}</td>
