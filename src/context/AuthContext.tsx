...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function registerFirstAccess(
    ...
    if (!supplierExists(supplier_id)) {
      return { success: false, message: "Código de miembro no encontrado o inactivo." };
    }

    if (!isFirstAccess(supplier_id)) {
      return { success: false, message: "Este miembro ya tiene una contraseña configurada. Use el acceso normal." };
    }
  ...
  async function login(supplier_id: string, password: string): Promise<{ success: boolean; message: string }> {
    ...
    if (!found || found.password !== password) {
      return { success: false, message: "Código de miembro o contraseña incorrectos." };
    }
